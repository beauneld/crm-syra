import { Search, ArrowLeft, Save, Paperclip, Plus, FileText, Trash2, Download, X, CreditCard as Edit2, MessageSquare } from 'lucide-react';
...
interface FormData {
  ...
  status: string;
  signature_type?: 'immediate' | 'email' | null;
  ...
  statut_professionnel_conjoint: string;
  profession_conjoint: string;
  dividende_conjoint: string;
  remuneration_conjoint: string;
  origine: string;
...
export default function NewDevoirConseil({ onClose, onSubmit, initialData, conseilId }: NewDevoirConseilProps) {
  ...
  const [activeTab, setActiveTab] = useState<'GENERAL' | 'FAMILLE' | 'BESOINS' | 'APPORTEUR' | 'COMMENTAIRE'>('GENERAL');
  ...
  const [familleModified, setFamilleModified] = useState(false);
  const [besoinsModified, setBesoinsModified] = useState(false);
  ...
  useEffect(() => {
    const loadUserProfile = async () => {
      ...
      if (profilesData) {
        const role = profilesData.profile_type;
        setUserRole(role);
        const hasFullAccess = ['Admin', 'Manager', 'Gestion'].includes(role);
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    status: initialData?.status || 'Non signé',
    ...
    regime_matrimonial: initialData?.regime_matrimonial || '',
    date_mariage: initialData?.date_mariage || '',
    testament: initialData?.testament || false,
    donation_entre_epoux: initialData?.donation_entre_epoux || false,
    conjoint_civilite: initialData?.conjoint_civilite || 'M.',
    conjoint_nom: initialData?.conjoint_nom || '',
    conjoint_prenom: initialData?.conjoint_prenom || '',
    conjoint_date_naissance: initialData?.conjoint_date_naissance || '',
    statut_professionnel_conjoint: initialData?.statut_professionnel_conjoint || '',
    profession_conjoint: initialData?.profession_conjoint || '',
    dividende_conjoint: initialData?.dividende_conjoint || '0',
    remuneration_conjoint: initialData?.remuneration_conjoint || '0',
    origine: initialData?.origine || '',
    description_besoin: initialData?.description_besoin || '',
  });
    ...
    const loadContracts = async () => {
      ...
      if (error) {
        ...
        const mappedContracts = data.map(contract => ({
          type_contrat: contract.contrat_type,
          gamme_contrat: contract.contrat_nom,
          assureur: contract.contrat_type.includes('PER') ? 'Suravenir' : 'Generali',
          en_portefeuille: true,
          garanties: contract.garanties,
          exclusions: contract.exclusions,
          limites: contract.limites,
          conditions: contract.conditions,
          options: contract.options,
          montants_garantie: contract.montants_garantie
        }));
  ...
  // Load propositions from initialData
  useEffect(() => {
    if (initialData?.produits_proposes) {
      const loadedPropositions = initialData.produits_proposes
        .split(',')
        .map(p => p.trim())
        .filter(p => p.length > 0);
      setPropositions(loadedPropositions);
    }
  }, [initialData?.produits_proposes]);
  ...
  const handleValidateSignature = async (type: 'immediate' | 'email') => {
    try {
      ...
      const devoirData = {
        ...
        status: type === 'immediate' ? 'Signature physique' : 'Signature par email',
        signature_type: type,
        user_id: userId
      };

      let savedConseilId = conseilId;

      if (conseilId) {
        // Update existing devoir
        ...
        if (error) {
          ...
          alert('Erreur lors de la mise à jour du devoir de conseil');
          return;
        }

        // Delete existing contracts for this devoir
        await supabase
          .from('devoir_conseil_contrats')
          .delete()
          .eq('devoir_conseil_id', conseilId);
        ...
        savedConseilId = data?.id;
      }

      // Save contracts to devoir_conseil_contrats table
      if (savedConseilId && contracts.length > 0) {
        const contractsToSave = contracts.map(contract => ({
          devoir_conseil_id: savedConseilId,
          contrat_type: contract.type_contrat || '',
          contrat_nom: contract.gamme_contrat || '',
          garanties: contract.garanties || '',
          exclusions: contract.exclusions || '',
          limites: contract.limites || '',
          conditions: contract.conditions || '',
        ...
        if (contractsError) {
          console.error('Erreur lors de la sauvegarde des contrats:', contractsError);
        }
      }
      ...
      if (type === 'immediate') {
        alert(conseilId ? 'Devoir de conseil mis à jour avec succès !' : 'Devoir de conseil enregistré avec succès !');
      } else {
        alert(conseilId ? 'Devoir de conseil mis à jour et envoyé au client par email !' : 'Devoir de conseil enregistré et envoyé au client par email !');
      }
    ...
    <div className="space-y-4 animate-fadeIn">
      ...
      <div className="glass-card floating-shadow px-6 py-3">
        <div className="flex gap-2 overflow-x-auto">
          {([{key: 'GENERAL', label: 'Général'}, {key: 'FAMILLE', label: 'Famille'}, {key: 'BESOINS', label: 'Besoins'}, {key: 'APPORTEUR', label: "Apporteur d'affaires"}, {key: 'COMMENTAIRE', label: 'Commentaire'}] as const)
            .filter(tab => canEditAll || tab.key === 'GENERAL')
            .map((tab) => (
            <button
              key={tab.key}
      ...
      <div className="glass-card floating-shadow p-8">
          {activeTab === 'GENERAL' && (
            <div className="space-y-6">
              ...
              <div className="search-container relative">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={!canEditAll}
                    className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 disabled:bg-gray-100 disabled:cursor-not-allowed"
                  />
                </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <label className={`relative inline-flex items-center ${canEditAll ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
                  <input
                    type="checkbox"
              ...
              <div className="grid grid-cols-5 gap-4">
                <div>
                  ...
                  <select
                    ...
                    disabled={!canEditAll}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 bg-white disabled:bg-gray-100 disabled:cursor-not-allowed"
                  >
                    <option>M.</option>
                    <option>Mme.</option>
                  </select>
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={!canEditAll}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 disabled:bg-gray-100 disabled:cursor-not-allowed"
                  />
                </div>
                <div>
                  <label className="block text-sm font-normal text-gray-700 mb-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-end">
                  <label className={`flex items-center gap-2 ${canEditAll ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  disabled={!canEditAll}
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 resize-none disabled:bg-gray-100 disabled:cursor-not-allowed"
                />
              </div>
          ...
          {activeTab === 'FAMILLE' && (
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-normal text-gray-700 mb-2">Situation familiale</label>
                  <select
                    value={formData.situation_familiale}
                    onChange={(e) => {
                      updateFormField('situation_familiale', e.target.value);
                      setFamilleModified(true);
                    }}
                ...
                <div>
                  <label className="block text-sm font-normal text-gray-700 mb-2">Régime matrimonial</label>
                  <select
                    value={formData.regime_matrimonial}
                    onChange={(e) => {
                      updateFormField('regime_matrimonial', e.target.value);
                      setFamilleModified(true);
                    }}
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 bg-white"
                  >
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-normal text-gray-700 mb-2">Date de mariage</label>
                  <input
                    type="date"
                    value={formData.date_mariage}
                    onChange={(e) => {
                      updateFormField('date_mariage', e.target.value);
                      setFamilleModified(true);
                    }}
                ...
                <div className="flex items-center pt-7">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.testament}
                      onChange={(e) => {
                        updateFormField('testament', e.target.checked);
                        setFamilleModified(true);
                      }}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                    />
                    <span className="text-sm font-normal text-gray-700">Testament</span>
                  </label>
                </div>
                <div className="flex items-center pt-7">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.donation_entre_epoux}
                      onChange={(e) => {
                        updateFormField('donation_entre_epoux', e.target.checked);
                        setFamilleModified(true);
                      }}
                      className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-2 focus:ring-blue-500"
                    />
                    <span className="text-sm font-normal text-gray-700">Donation entre époux</span>
                  </label>
                </div>
              </div>

              <div className="border-t border-gray-200 pt-6">
                <h3 className="text-base font-medium text-gray-900 mb-4">Conjoint</h3>
                <div className="grid grid-cols-4 gap-4 mb-4">
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Civilité</label>
                    <select
                      value={formData.conjoint_civilite}
                      onChange={(e) => {
                        updateFormField('conjoint_civilite', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 bg-white"
                    >
                  ...
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Nom conjoint</label>
                    <input
                      type="text"
                      value={formData.conjoint_nom}
                      onChange={(e) => {
                        updateFormField('conjoint_nom', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
                  ...
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Prénom conjoint</label>
                    <input
                      type="text"
                      value={formData.conjoint_prenom}
                      onChange={(e) => {
                        updateFormField('conjoint_prenom', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
                  ...
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Date de naissance conjoint</label>
                    <input
                      type="date"
                      value={formData.conjoint_date_naissance}
                      onChange={(e) => {
                        updateFormField('conjoint_date_naissance', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Statut professionnel</label>
                    <select
                      value={formData.statut_professionnel_conjoint}
                      onChange={(e) => {
                        updateFormField('statut_professionnel_conjoint', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400 bg-white"
                    >
                  ...
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Profession conjoint</label>
                    <input
                      type="text"
                      value={formData.profession_conjoint}
                      onChange={(e) => {
                        updateFormField('profession_conjoint', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      value={formData.dividende_conjoint}
                      onChange={(e) => {
                        updateFormField('dividende_conjoint', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
                  ...
                  <div>
                    <label className="block text-sm font-normal text-gray-700 mb-2">Rémunération</label>
                    <input
                      type="number"
                      value={formData.remuneration_conjoint}
                      onChange={(e) => {
                        updateFormField('remuneration_conjoint', e.target.value);
                        setFamilleModified(true);
                      }}
                      className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                    />
              ...
              <div className="border-t border-gray-200 pt-6">
                <button
                  type="button"
                  onClick={() => {
                    setChildren([...children, { civilite: 'M.', nom: '', prenom: '', date_naissance: '' }]);
                    setFamilleModified(true);
                  }}
                  className="px-6 py-2 text-blue-600 border border-blue-300 rounded-full text-sm font-normal hover:bg-blue-50 transition-colors"
                >
                  + Ajouter un enfant
                </button>
                ...
                {children.length > 0 && (
                  <div className="space-y-3 mt-4">
                    {children.map((child, index) => (
                      <div key={index} className="grid grid-cols-5 gap-2 items-end p-3 bg-gray-50 rounded-lg">
                        <select
                          value={child.civilite}
                          onChange={(e) => {
                            const newChildren = [...children];
                            newChildren[index].civilite = e.target.value;
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newChildren = [...children];
                            newChildren[index].nom = e.target.value;
                            setChildren(newChildren);
                            setFamilleModified(true);
                          }}
                          placeholder="Nom"
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newChildren = [...children];
                            newChildren[index].prenom = e.target.value;
                            setChildren(newChildren);
                            setFamilleModified(true);
                          }}
                          placeholder="Prénom"
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newChildren = [...children];
                            newChildren[index].date_naissance = e.target.value;
                            setChildren(newChildren);
                            setFamilleModified(true);
                          }}
                          className="px-2 py-1.5 border border-gray-300 rounded text-sm"
              ...
              <div className="flex justify-center pt-6 border-t border-gray-200">
                <button
                  type="button"
                  onClick={() => setFamilleModified(false)}
                  className="px-8 py-2.5 border border-blue-300 text-blue-600 rounded-full font-normal hover:bg-blue-50 transition-colors"
                >
                  Sauvegarder
                </button>
              </div>
          ...
          {activeTab === 'BESOINS' && (
            <div className="space-y-6">
              <div>
                <label className="block text-sm font-normal text-gray-700 mb-2">Origine</label>
                <input
                  type="text"
                  value={formData.origine}
                  onChange={(e) => {
                    updateFormField('origine', e.target.value);
                    setBesoinsModified(true);
                  }}
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                />
              </div>
              ...
              <div>
                <label className="block text-sm font-normal text-gray-700 mb-2">Besoin</label>
                <input
                  type="text"
                  value={formData.besoins}
                  onChange={(e) => {
                    updateFormField('besoins', e.target.value);
                    setBesoinsModified(true);
                  }}
                  className="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm font-normal focus:outline-none focus:ring-2 focus:ring-blue-400/50 focus:border-blue-400"
                />
              ...
              <div>
                <label className="block text-sm font-normal text-gray-700 mb-2">Description du besoin</label>
                <textarea
                  value={formData.description_besoin}
                  onChange={(e) => {
                    updateFormField('description_besoin', e.target.value);
                    setBesoinsModified(true);
                  }}
                  rows={6}
              ...
              <div className="flex justify-center pt-6 border-t border-gray-200">
                <button
                  type="button"
                  onClick={() => setBesoinsModified(false)}
                  className="px-8 py-2.5 border border-blue-300 text-blue-600 rounded-full font-normal hover:bg-blue-50 transition-colors"
                >
                  Sauvegarder
                </button>
      ...
      <div className="glass-card floating-shadow p-8">
        <p className="text-sm font-light text-gray-600 mb-6">
          Format : JPEG, PNG, PDF - Poids : 5Mo maximum
          {!canEditAll && <span className="ml-2 text-orange-600">(Mode consultation uniquement)</span>}
        </p>
        ...
        <div className="space-y-6">
          <div className="flex items-start justify-between py-4 border-b border-gray-200">
            ...
            <div className="flex items-center gap-2">
              {canEditAll && (
                <label className="px-4 py-2 bg-white border border-blue-500 text-blue-600 rounded-full text-sm font-light hover:bg-blue-50 transition-all cursor-pointer flex items-center gap-2">
              ...
              {hasAttachment && (
                <button className="px-4 py-2 bg-gray-100 text-gray-600 rounded-full text-sm font-light hover:bg-gray-200 transition-all flex items-center gap-2">
          ...
          <div className="flex items-start justify-between py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-sm font-light text-gray-600">0 document(s)</span>
            </div>
          </div>
        </div>
      </div>
