/*
  # Configuration complète de la base de données CRM
  
  1. Tables créées
    - profiles: Profils utilisateurs liés à auth.users
    - user_profiles: Profils utilisateurs avec rôles et équipes
  ...
  2. Sécurité
    ...
    - Politiques d'accès adaptées par rôle
    - Buckets de stockage configurés
    
  3. Données de démonstration
    - Profils utilisateurs initiaux
    - Configuration organisation par défaut
*/
...
DROP POLICY IF EXISTS "Public write access for user_profiles" ON user_profiles;
CREATE POLICY "Public write access for user_profiles"
  ON user_profiles FOR INSERT
  TO public
  WITH CHECK (true);

DROP POLICY IF EXISTS "Public update access for user_profiles" ON user_profiles;
...
DROP POLICY IF EXISTS "Public read access for devoirs conseil" ON devoirs_conseil;
CREATE POLICY "Public read access for devoirs conseil"
  ON devoirs_conseil
  FOR SELECT
  TO public
  USING (true);

DROP POLICY IF EXISTS "Public create access for devoirs conseil" ON devoirs_conseil;
CREATE POLICY "Public create access for devoirs conseil"
  ON devoirs_conseil
  FOR INSERT
  TO public
  WITH CHECK (true);

DROP POLICY IF EXISTS "Public update access for devoirs conseil" ON devoirs_conseil;
CREATE POLICY "Public update access for devoirs conseil"
  ON devoirs_conseil
  FOR UPDATE
  TO public
  USING (true)
...
DROP POLICY IF EXISTS "Public delete access for devoirs conseil" ON devoirs_conseil;
CREATE POLICY "Public delete access for devoirs conseil"
  ON devoirs_conseil
  FOR DELETE
  TO public
  USING (true);
...
CREATE TABLE IF NOT EXISTS organization_settings (
  ...
  main_logo_dark_url text,
  collapsed_logo_dark_url text,
...
CREATE POLICY "Public read access for memos"
  ON memos FOR SELECT
  TO public
  USING (true);

DROP POLICY IF EXISTS "Public create access for memos" ON memos;
CREATE POLICY "Public create access for memos"
  ON memos FOR INSERT
  TO public
  WITH CHECK (true);

DROP POLICY IF EXISTS "Public update access for memos" ON memos;
CREATE POLICY "Public update access for memos"
  ON memos FOR UPDATE
  TO public
  USING (true)
  WITH CHECK (true);

DROP POLICY IF EXISTS "Public delete access for memos" ON memos;
CREATE POLICY "Public delete access for memos"
  ON memos FOR DELETE
  TO public
  USING (true);
